...
export default function Testimonials() {
  ...
  return <section id="testimonials" className="section-padding bg-burgundy/5">
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          <span className="text-burgundy font-medium">Client Stories</span>
          <h2 className="section-title">Testimonials</h2>
          <p className="section-subtitle mx-auto">Here's what our valued clients have to say about their experience with Rebekha Caterers.</p>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="relative z-10">
              <div className="flex flex-col md:flex-row md:items-center gap-6 md:gap-10 mb-8">
                <div className="flex-shrink-0">
                  <img src={testimonials[activeIndex].image} alt={testimonials[activeIndex].name} className="w-20 h-20 rounded-full object-cover" />
                </div>
                <div>
                  <h3 className="font-serif font-bold text-xl text-burgundy">{testimonials[activeIndex].name}</h3>
                  <p className="text-gray-600">{testimonials[activeIndex].role}</p>
                  <div className="flex mt-2">
                    {[...Array(5)].map((_, i) => <svg key={i} xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill={i < testimonials[activeIndex].rating ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.5" className={i < testimonials[activeIndex].rating ? "text-yellow-500" : "text-gray-300"}>
          ...
          <div className="flex justify-center gap-3">
            {testimonials.map((_, index) => <button key={index} onClick={() => setActiveIndex(index)} className={`w-3 h-3 rounded-full transition-all ${index === activeIndex ? "bg-burgundy scale-125" : "bg-burgundy/30"}`} aria-label={`Go to testimonial ${index + 1}`} />)}
          </div>
          
          <div className="flex justify-center mt-8 gap-4">
            <button onClick={handlePrev} className="p-2 rounded-full bg-white shadow-md hover:bg-burgundy/5 transition-colors" aria-label="Previous testimonial">
            ...
            <button onClick={handleNext} className="p-2 rounded-full bg-white shadow-md hover:bg-burgundy/5 transition-colors" aria-label="Next testimonial">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-burgundy">
